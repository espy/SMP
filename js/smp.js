...
function redrawLayout() {
  ...
  // Dock to top right corner of WRAPPER
  //$('#backgroundImage, .nextNavi').css('right', ($(window).width()/2) - (1170/2));
  // Dock to top right corner of WINDOW
  ...
  $('.viewport').css('background-position', $('.viewport').offset().left);
  //$('.viewport').height(viewportHeight);
}

function scaleBGImage() {
  var bgImage = $('#backgroundImage img')[0];
  // Targetwidth for wrapper
  // var targetWidth = 1115;
  // Targetwidth for window
  var targetWidth = ($(window).width() - ($(window).width() - 1115)/2)-10;
  var dimensions = getFitAroundSizes($('#backgroundImage img'), targetWidth, $(window).height());
