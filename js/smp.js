...
function redrawLayout() {
  ...
  $('html.ie.lte9').each(function(){
    var leftMargin = ($(window).width()-wrapperMaxWidth)/2;
  ...
  if($('.viewport.press').length > 0){
    ...
    var newsItemViewHeight = $('.viewport>.newsItemView').outerHeight();
...
function getHashes() {
  ...
  var url = state.url;
  hashes = state.hashedURL;
  // this removes an IE fuckup
  //hashes = hashes.replace("#.", "");
  var root = window.location.hostname;
  hashes = url.replace("http://", "");
  hashes = hashes.replace(root+"/", "").split("/");
  hashes = _.compact(hashes);
}
